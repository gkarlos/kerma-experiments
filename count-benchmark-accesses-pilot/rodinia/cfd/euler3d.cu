...
__global__ void cuda_compute_step_factor(int nelr, float *variables,
  ...
  float3 velocity;
  compute_velocity(density, momentum, velocity); // 0
  float speed_sqd = compute_speed_sqd(velocity); // 0
  float pressure = compute_pressure(density, density_energy, speed_sqd); // 0
  float speed_of_sound = compute_speed_of_sound(density, pressure); // 0
