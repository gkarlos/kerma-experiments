...
__device__ double d_randn(int *seed, int index) {
  // Box-Muller algortihm
  double pi = 3.14159265358979323846;
  double u = d_randu(seed, index); // 3
  double v = d_randu(seed, index); // 3
  double cosine = cos(2 * pi * v);
  double rt = -2 * log(u);
  return sqrt(rt) * cosine;
}
...
__global__ void normalize_weights_kernel(double *weights, int Nparticles,
  ...
  if (i == 0) {
    cdfCalc(CDF, weights, Nparticles); // 5/5/3
    u[0] =
        (1 / ((double)(Nparticles))) *
        d_randu( // 3/3
            seed,
...
__global__ void likelihood_kernel(double *arrayX, double *arrayY, double *xj,
  ...
  if (i < Nparticles) {
    ...
    arrayX[i] = arrayX[i] + 1.0 + 5.0 * d_randn(seed, i) /* 6 accesses*/;
    arrayY[i] = arrayY[i] - 2.0 + 2.0 * d_randn(seed, i) /* 6 accesses*/;
  }
  ...
  if (i < Nparticles) {
    ...
    // 28
    likelihood[i] = calcLikelihoodSum(I, ind, countOnes, i); // 1 + 4
    // 33
